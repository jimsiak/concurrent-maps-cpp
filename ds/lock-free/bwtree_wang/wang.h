/**
 * An open-source implementation of the Bw-tree.
 **/
...
/*
 * class BwTreeBase - Base class of bwtree_wang that stores some common members
 */
class BwTreeBase {
 ...
 public:
   ...
   * function inside bwtree_wang destructor
   *
 ...
 public: 
  ...
  static void RegisterThread() {
	...
	std::cout << "GOT GC_ID: " << gc_id << std::endl;
...
class bwtree_wang : public BwTreeBase, public Map<KeyType,ValueType> {
public:
	typedef KeyType K;
	typedef ValueType V;
...
//	}
	...
	bool  validate();
	char *name() { return "OpenBW-tree Wang"; }
...
//	unsigned long long size() { return size_rec(root); };
 ...
 public:
#endif
  // This does not have to be the friend class of bwtree_wang
  class EpochManager;
 ...
 public:
  ...
  class KeyNodeIDPairComparator {
   public:
     ...
     */
    KeyNodeIDPairComparator(bwtree_wang *p_tree_p) :
  ...
  class KeyNodeIDPairEqualityChecker {
   public:
     ...
     */
    KeyNodeIDPairEqualityChecker(bwtree_wang *p_tree_p) :
  ...
  class KeyNodeIDPairHashFunc {
   public:
     ...
     */
    KeyNodeIDPairHashFunc(bwtree_wang *p_tree_p) :
  ...
  class KeyValuePairComparator {
   public:
     ...
     * Constructor
     */
    KeyValuePairComparator(bwtree_wang *p_tree_p) :
      key_cmp_obj_p{&p_tree_p->key_cmp_obj}
    {}
  ...
  class KeyValuePairEqualityChecker {
   public:
     ...
     */
    KeyValuePairEqualityChecker(bwtree_wang *p_tree_p) :
      key_eq_obj_p{&p_tree_p->key_eq_obj},
  ...
  class KeyValuePairHashFunc {
   public:
     ...
     * Constructor - Initialize a key value pair hash function
     */
    KeyValuePairHashFunc(bwtree_wang *p_tree_p) :
      key_hash_obj_p{&p_tree_p->key_hash_obj},
   ...
   *
   * NOTE 2: We also store the InnerDeleteNode in bwtree_wang to facilitate
  ...
  class LeafNode : public ElasticNode<KeyValuePair> {
   public:
     ...
     * is returned
     */
    int FindSplitPoint(const bwtree_wang *t) const {
     ...
     * function is nullptr
     */
    LeafNode *GetSplitSibling(const bwtree_wang *t) const {
 ...
 public:
  /*
   * Constructor - Set up initial environment for bwtree_wang
   *
  ...
  bwtree_wang(const K _NO_KEY, const V _NO_VALUE, const int numProcesses,
    ...
    // We could choose not to start GC thread inside the bwtree_wang
	...
	UpdateThreadLocal(2*numProcesses);

    return;
  }

  /*
   * Destructor - Destroy bwtree_wang instance
   *
  ...
  /*
   * InitNodeLayout() - Initialize the nodes required to start bwtree_wang
   *
 ...
 public:
  ...
  class EpochManager {
   public:
     ...
     */
    EpochManager(bwtree_wang *p_tree_p) :
      tree_p{p_tree_p} {
...
try_join_again:
     ...
     * external threads calling the function while also allows bwtree_wang maintains
    ...
    /*
     * AddGarbageNode() - This encapsulates bwtree_wang::AddGarbageNode()
     */
     ...
     *
     * This function differs from the one of the same name in bwtree_wang definition
  ...
  class IteratorContext {
   private:
     ...
     */
    IteratorContext(bwtree_wang *p_tree_p) :
      tree_p{p_tree_p},
      ref_count{0UL} 
    {}
   ...
   public:
     ...
     */
    inline static IteratorContext *Get(bwtree_wang *p_tree_p, 
                                       const BaseNode *node_p) {
  ...
  class ForwardIterator {
   ...
   public:
     ...
     * know it is there
     */
    ForwardIterator(bwtree_wang *p_tree_p) {
     ...
     * instance
     */
    void LowerBound(bwtree_wang *p_tree_p,
      ...
      while(1) {  
        ...
        NodeSnapshot *snapshot_p = bwtree_wang::GetLatestNodeSnapshot(&context);
        const BaseNode *node_p = snapshot_p->node_p;
        assert(node_p->IsOnLeafDeltaChain() == true);
    ...
    void MoveBackByOne() {
      ...
      // This will be used to call bwtree_wang functions
      bwtree_wang *tree_p = ic_p->GetTree();
...
#define BWTRE_WANG_TEMPL template<typename K, typename V, \
          typename KeyComparator, \
		  typename KeyEqualityChecker, \
		  typename KeyHashFunc, \
		  typename ValueEqualityChecker, \
		  typename ValueHashFunc>
#define BWTREE_WANG_FUNCT bwtree_wang<K,V,KeyComparator,KeyEqualityChecker,KeyHashFunc,ValueEqualityChecker,ValueHashFunc>

BWTRE_WANG_TEMPL
bool BWTREE_WANG_FUNCT::contains(const int tid, const K& key)
{
	std::vector<V> val;
	GetValue(key, val);
	return val.size() > 0;
...
BWTRE_WANG_TEMPL
const std::pair<V,bool> BWTREE_WANG_FUNCT::find(const int tid, const K& key)
{
	std::vector<V> val;
	GetValue(key, val);
	if (val.size() > 0) return std::pair<V,bool>(val[0], true);
	else                return std::pair<V,bool>(NULL, false);
}

BWTRE_WANG_TEMPL
int BWTREE_WANG_FUNCT::rangeQuery(const int tid, const K& lo, const K& hi,
                      std::vector<std::pair<K,V>> kv_pairs)
{
	return 0;
}
...
BWTRE_WANG_TEMPL
const V BWTREE_WANG_FUNCT::insert(const int tid, const K& key, const V& val)
{
...
BWTRE_WANG_TEMPL
bool BWTREE_WANG_FUNCT::validate()
{
