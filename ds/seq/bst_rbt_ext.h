...
public:
	...
	bool  validate();
	char *name() { return "BST Red-Black External"; }
...
private:
	              ...
	              node_t *node_stack[MAX_HEIGHT], int stack_top)
	{
		...
		parent->left = new node_t(key, value, BLACK);
	...
	int do_delete(const K& key, node_t *node_stack[MAX_HEIGHT],
	              int *stack_top, color_t *deleted_node_color)
	{
		// Empty tree
		...
		*deleted_node_color = parent->color;
		...
		(*stack_top)--;
		if (key <= parent->key) node_stack[*stack_top] = parent->right;
		else                    node_stack[*stack_top] = parent->left;
	...
	void print_rec(node_t *root, int level)
	{
		...
		printf("%llu[%s]\n", root->key, (root->color == RED) ? "RED" : "BLA");
...
#define BST_RBT_EXT_TEMPL template<typename K, typename V>
#define BST_RBT_EXT_FUNCT bst_rbt_ext<K,V>

BST_RBT_EXT_TEMPL
bool BST_RBT_EXT_FUNCT::contains(const int tid, const K& key)
{
	return lookup_helper(key);
}

BST_RBT_EXT_TEMPL
const std::pair<V,bool> BST_RBT_EXT_FUNCT::find(const int tid, const K& key)
{
...
BST_RBT_EXT_TEMPL
int BST_RBT_EXT_FUNCT::rangeQuery(const int tid, const K& lo, const K& hi,
                      std::vector<std::pair<K,V>> kv_pairs)
{
	return 0;
}
...
BST_RBT_EXT_TEMPL
const V BST_RBT_EXT_FUNCT::insert(const int tid, const K& key, const V& val)
{
...
BST_RBT_EXT_TEMPL
bool BST_RBT_EXT_FUNCT::validate()
{
