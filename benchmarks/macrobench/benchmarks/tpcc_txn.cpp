...
RC tpcc_txn_man::run_payment(tpcc_query * query)
{
	...
	row_t * r_wh_local;
	if (g_wh_update) r_wh_local = get_row(r_wh, WR);
	else             r_wh_local = get_row(r_wh, RD);
	if (r_wh_local==NULL) return finish(Abort);

	double w_ytd;

	r_wh_local->get_value(W_YTD, w_ytd);
	if (g_wh_update) r_wh_local->set_value(W_YTD, w_ytd+query->h_amount);
		...
		for (n=0; n<namecnt/2; n++) {
			EXEC SQL FETCH c_byname
			         INTO :c_first, :c_middle, :c_id,
			              ...
			              :c_phone, :c_credit, :c_credit_lim, :c_discount,
			              :c_balance, :c_since;
		}
...
//	insert_row(r_no, _wl->t_neworder); 
	...
	for (UInt32 ol_number = 0; ol_number<ol_cnt; ol_number++) {
		...
		uint64_t quantity;
		if (s_quantity>ol_quantity+10) quantity = s_quantity-ol_quantity;
		else                           quantity = s_quantity-ol_quantity+91;
