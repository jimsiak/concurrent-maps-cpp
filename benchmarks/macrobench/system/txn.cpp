...
int txn_man::index_range_query(Index *index, idx_key_t low, idx_key_t high,
...
RC txn_man::finish(RC rc)
{
	...
	uint64_t starttime = get_sys_clock();
	#if CC_ALG == OCC
	if (rc == RCOK) rc = occ_man.validate(this);
	else            cleanup(rc);
	#elif CC_ALG == TICTOC
	if (rc == RCOK) rc = validate_tictoc();
	else            cleanup(rc);
	#elif CC_ALG == SILO
	if (rc == RCOK) rc = validate_silo();
	else            cleanup(rc);
	#elif CC_ALG == HEKATON
