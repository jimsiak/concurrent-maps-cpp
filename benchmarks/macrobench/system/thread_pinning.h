...
static unsigned parseToken(std::string argv, int ix)
{
	...
	return (ix >= argv.size() ? argv.size() : ix+1);
}
...
static void parseCustom(std::string argv)
{
	numCustomBindings = 0;
	unsigned ix = 0;
	while (ix < argv.size()) ix = parseToken(argv, ix);
}

static void doBindThread(const int tid, const int nprocessors)
{
	if (sched_setaffinity(0, CPU_ALLOC_SIZE(nprocessors),
	                      cpusets[tid%nprocessors])) {
		cout << "ERROR: could not bind thread " << tid << " to cpuset "
		     << cpusets[tid%nprocessors] << std::endl;
		exit(-1);
	}
}
