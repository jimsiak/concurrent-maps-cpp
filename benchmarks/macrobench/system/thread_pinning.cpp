...
{
	if (numCustomBindings > 0) doBindThread(tid, LOGICAL_PROCESSORS);
}

int getActualBinding(const int tid)
{
	int result = -1;
	if (numCustomBindings == 0) return result;

	unsigned bindings = 0;
	for (int i=0; i < LOGICAL_PROCESSORS; ++i) {
		if (CPU_ISSET_S(i, CPU_ALLOC_SIZE(LOGICAL_PROCESSORS),
		                cpusets[tid%LOGICAL_PROCESSORS])) {
			result = i;
			++bindings;
		}
	}
...
bool isInjectiveMapping(const int numThreads)
{
	...
	if (numCustomBindings == 0) return true;
